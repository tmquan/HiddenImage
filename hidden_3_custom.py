	...
	def _build_graph(self, inputs):
		...
		with tf.name_scope('membr_loss'):
			def membr_loss(y_true, y_pred, name='membr_loss'):
				return tf.subtract(binary_cross_entropy(cvt2imag(y_true, maxVal=1.0), cvt2imag(y_pred, maxVal=1.0)), 
								   dice_coe(cvt2imag(y_true, maxVal=1.0), cvt2imag(y_pred, maxVal=1.0), axis=[1,2,3], loss_type='jaccard'),  name='membr_im')
